...
public class ProductsDao {
    ...
    public boolean registerProductQuery(Products product){
        String query ="INSERT INTO products(code,name,description,unit_price,"
                + "created,updated,category_id) "
                + " VALUES(?,?,?,?,?,?,?)";
    ...
    public Products searchProduct(int id){
        String query = "SELECT pro.*, ca.name AS category_name, pro.product_quantity FROM products pro INNER JOIN categories ca ON pro.category_id = ca.id WHERE pro.id = ? ";
        Products product = new Products();
